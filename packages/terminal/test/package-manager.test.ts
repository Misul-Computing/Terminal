...
describe("DefaultPackageManager", () => {
	...
	beforeEach(() => {
		previousOfflineEnv = process.env.MISUL_OFFLINE;
	...
	afterEach(() => {
		if (previousOfflineEnv === undefined) {
			...
			process.env.MISUL_OFFLINE = previousOfflineEnv;
		}
		vi.restoreAllMocks();
		vi.unstubAllGlobals();
...
Content`,
		...
		it("should auto-discover root markdown skills from .misul skill dirs", async () => {
...
Content`,
		...
		it("should resolve project paths relative to .misul", async () => {
			const extDir = join(tempDir, ".misul", "extensions");
			mkdirSync(extDir, { recursive: true });
...
Content`,
				...
				mkdirSync(join(tempDir, ".misul"), { recursive: true });
				...
				symlinkSync(sharedExtensionsDir, join(tempDir, ".misul", "extensions"), "dir");
				symlinkSync(sharedSkillsDir, join(tempDir, ".misul", "skills"), "dir");
				symlinkSync(sharedPromptsDir, join(tempDir, ".misul", "prompts"), "dir");
				symlinkSync(sharedThemesDir, join(tempDir, ".misul", "themes"), "dir");
		...
		it("should auto-discover project prompts with overrides", async () => {
			const promptsDir = join(tempDir, ".misul", "prompts");
			mkdirSync(promptsDir, { recursive: true });
		...
		it("should resolve directory with package.json misul.extensions in extensions setting", async () => {
			// Create a package with misul.extensions in package.json
			...
			// Should find the extensions declared in package.json misul.extensions
	...
	describe("auto-discovered skill metadata", () => {
		it("should use the agent dir as baseDir for user .misul/agent skills", async () => {
			const skillPath = join(agentDir, "skills", "user-misul", "SKILL.md");
			mkdirSync(join(agentDir, "skills", "user-misul"), { recursive: true });
			writeFileSync(skillPath, "---\nname: user-misul\ndescription: user misul\n---\n");
		...
		it("should use the project .misul dir as baseDir for project .misul skills", async () => {
			const projectBaseDir = join(tempDir, ".misul");
			const skillPath = join(projectBaseDir, "skills", "project-misul", "SKILL.md");
			mkdirSync(join(projectBaseDir, "skills", "project-misul"), { recursive: true });
			writeFileSync(skillPath, "---\nname: project-misul\ndescription: project misul\n---\n");
	...
	describe(".agents/skills auto-discovery", () => {
		...
		it("should keep ~/.agents/skills user-scoped when cwd is under home in a non-git directory", async () => {
			...
			try {
				...
				const localAgentDir = join(tempDir, ".misul", "agent");
		...
		it("should dedupe user skill entries when ~/.misul/agent/skills is a symlink to ~/.agents/skills", async () => {
			const previousHome = process.env.HOME;
			process.env.HOME = tempDir;
	...
	describe("ignore files", () => {
		...
		it("should not apply parent .gitignore to .misul auto-discovery", async () => {
			writeFileSync(join(tempDir, ".gitignore"), ".misul\n");

			const skillDir = join(tempDir, ".misul", "skills", "auto-skill");
			mkdirSync(skillDir, { recursive: true });
	...
	describe("resolveExtensionSources", () => {
		...
		it("should handle directories with misul manifest", async () => {
		...
		it("should keep misul manifest entries with leading tilde package-relative", async () => {
	...
	describe("command spawning", () => {
		it("should preserve argv entries containing spaces", () => {
			...
			const valueWithSpace = "C:\\Users\\A B\\.misul\\npm";
	...
	describe("npmCommand", () => {
		...
		it("should update git package dependencies with --omit=dev", async () => {
			const source = "git:github.com/user/repo";
			const targetDir = join(tempDir, ".misul", "git", "github.com", "user", "repo");
			mkdirSync(targetDir, { recursive: true });
			writeFileSync(join(targetDir, "package.json"), JSON.stringify({ name: "repo", version: "1.0.0" }));
		...
		it("should install user npm packages into the misul-managed npm root", async () => {
			settingsManager = SettingsManager.inMemory({
				npmCommand: ["pnpm"],
	...
	describe("temporary install paths", () => {
		it("should place temporary npm packages under the agent temp extension folder", () => {
			...
			expect(installPath.startsWith(join(tmpdir(), "misul-extensions"))).toBe(false);
	...
	describe("settings source normalization", () => {
		...
		it("should store project local packages relative to .misul settings base", () => {
			...
			const rel = relative(join(tempDir, ".misul"), projectPkgDir);
			const expected = rel.startsWith(".") ? rel : `./${rel}`;
			expect(settings.packages?.[0]).toBe(expected);
		});
	...
	describe("pattern filtering in misul manifest", () => {
...
export default function(api) { api.registerTool({ name: "test", description: "test", execute: async () => helper() }); }`,
		...
		it("should respect package.json misul.extensions manifest in subdirectories", async () => {
	...
	describe("offline mode and network timeouts", () => {
		it("should update npm range packages using the configured spec", async () => {
			const installedPath = join(tempDir, ".misul", "npm", "node_modules", "example");
			...
			expect(runCommandSpy).toHaveBeenCalledWith(
				"npm",
				["install", "example@^1.0.0", "--prefix", join(tempDir, ".misul", "npm"), "--legacy-peer-deps"],
				undefined,
			);
		});
		...
		it("should batch npm updates per scope and run git updates in parallel while skipping pinned npm and current packages", async () => {
			...
			const projectOldPath = join(tempDir, ".misul", "npm", "node_modules", "project-old");
			const projectCurrentPath = join(tempDir, ".misul", "npm", "node_modules", "project-current");
		...
		it("should skip pinned packages when checking for updates", async () => {
			const installedNpmPath = join(tempDir, ".misul", "npm", "node_modules", "example");
