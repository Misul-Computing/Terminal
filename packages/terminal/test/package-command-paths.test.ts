...
describe("package commands", () => {
	...
	let originalMisulPackageDir: string | undefined;
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `misul-package-commands-${Date.now()}-${Math.random().toString(36).slice(2)}`);
		...
		originalMisulPackageDir = process.env.MISUL_PACKAGE_DIR;
	...
	it("skips untrusted project package settings", async () => {
		mkdirSync(join(projectDir, ".misul"), { recursive: true });
		writeFileSync(join(projectDir, ".misul", "settings.json"), JSON.stringify({ packages: ["npm:@project/pkg"] }));
		...
		try {
			await expect(
				main(["list"], {
					extensionFactories: [
						(misul) => {
							misul.on("project_trust", () => ({ trusted: "yes" }));
						},
					],
				}),
	...
	it("does not prompt or ask extensions for project trust during update", async () => {
		...
		writeFileSync(
			join(projectDir, ".misul", "settings.json"),
	...
	it("blocks local package changes when project is untrusted", async () => {
		mkdirSync(join(projectDir, ".misul"), { recursive: true });
		writeFileSync(join(projectDir, ".misul", "settings.json"), "{}");
	...
	it("allows local package install to initialize fresh project settings", async () => {
		...
		const settingsPath = join(projectDir, ".misul", "settings.json");
		const settings = JSON.parse(readFileSync(settingsPath, "utf-8")) as { packages?: string[] };
		expect(settings.packages?.length).toBe(1);
		const stored = settings.packages?.[0] ?? "";
		expect(realpathSync(join(projectDir, ".misul", stored))).toBe(realpathSync(packageDir));
		expect(process.exitCode).toBeUndefined();
	});
	...
	it("shows install subcommand help", async () => {
		...
		try {
			...
			expect(stdout).toContain("misul install <source> [-l]");
	...
	it("shows a friendly error for unknown install options", async () => {
		...
		try {
			...
			expect(stderr).toContain('Use "misul --help" or "misul install <source> [-l] [--approve|--no-approve]".');
			expect(process.exitCode).toBe(1);
		} finally {
			errorSpy.mockRestore();
		}
	...
	it("shows a friendly error for missing install source", async () => {
		...
		try {
			...
			expect(stderr).toContain("Usage: misul install <source> [-l]");
	...
	it("uses global npmCommand and current package name for forced self updates without checking the api", async () => {
		...
		const selfPackageDir = join(globalPrefix, "lib", "node_modules", "@earendil-works", "misul-coding-agent");
...
`,
		...
		process.env.MISUL_PACKAGE_DIR = selfPackageDir;
	...
	it("self-update without --force reports up to date and never polls or switches packages", async () => {
		const globalPrefix = join(tempDir, "global-prefix");
		const selfPackageDir = join(globalPrefix, "lib", "node_modules", "@mariozechner", "misul-coding-agent");
		const fakeNpmPath = join(tempDir, "fake-npm.cjs");
		const recordPath = join(tempDir, "self-update.json");
		mkdirSync(selfPackageDir, { recursive: true });
		writeFileSync(
...
`,
		...
		// Even if a (foreign) release were reachable, Misul must not poll or switch packages.
		const fetchMock = vi.fn(async () => Response.json({ packageName: "@some-other/pkg", version: "99.0.0" }));
		vi.stubGlobal("fetch", fetchMock);
		...
		try {
			...
			expect(process.exitCode).toBeUndefined();
			expect(errorSpy).not.toHaveBeenCalled();
			// No release feed: never polls, never installs, never switches packages.
			expect(fetchMock).not.toHaveBeenCalled();
			const stdout = logSpy.mock.calls.map(([message]) => String(message)).join("\n");
	...
	it("suggests the configured source when update input omits the npm prefix", async () => {
		const settingsPath = join(agentDir, "settings.json");
		writeFileSync(settingsPath, JSON.stringify({ packages: ["npm:misul-formatter"] }, null, 2));

		const errorSpy = vi.spyOn(console, "error").mockImplementation(() => {});
		const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});

		try {
			await expect(main(["update", "misul-formatter"])).resolves.toBeUndefined();

			const stderr = errorSpy.mock.calls.map(([message]) => String(message)).join("\n");
			const stdout = logSpy.mock.calls.map(([message]) => String(message)).join("\n");
			expect(stderr).toContain("Did you mean npm:misul-formatter?");
			expect(stdout).not.toContain("Updated misul-formatter");
			expect(process.exitCode).toBe(1);

			const settings = JSON.parse(readFileSync(settingsPath, "utf-8")) as { packages?: string[] };
			expect(settings.packages).toContain("npm:misul-formatter");
		} finally {
			errorSpy.mockRestore();
			logSpy.mockRestore();
		}
