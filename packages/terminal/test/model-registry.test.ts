...
import type { AnthropicMessagesCompat, Api, Context, Model, OpenAICompletionsCompat } from "@misul/ai";
import { getApiProvider } from "@misul/ai";
import { getOAuthProvider } from "@misul/ai/oauth";
...
describe("ModelRegistry", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `misul-test-model-registry-${Date.now()}-${Math.random().toString(36).slice(2)}`);
