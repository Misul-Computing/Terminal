...
describe("theme picker", () => {
	...
	beforeEach(() => {
		tempRoot = mkdtempSync(join(tmpdir(), "misul-theme-picker-"));
		const agentDir = join(tempRoot, "agent");
		vi.stubEnv("MISUL_TERMINAL_DIR", agentDir);
	...
	it("uses custom theme content names instead of file names", () => {
		...
		const themePath = join(process.env.MISUL_TERMINAL_DIR!, "themes", "foo.json");
