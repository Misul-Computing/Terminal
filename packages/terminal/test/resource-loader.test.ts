...
Skill content here.`,
		...
		it("should ignore extra markdown files in auto-discovered skill dirs", async () => {
			const skillDir = join(agentDir, "skills", "misul-skills", "browser-tools");
...
Prompt content.`,
		...
		it("should prefer project resources over user on name collisions", async () => {
			...
			const projectPromptsDir = join(cwd, ".misul", "prompts");
			...
			const projectSkillDir = join(cwd, ".misul", "skills", "collision-skill");
...
Project skill`,
			...
			mkdirSync(join(cwd, ".misul", "themes"), { recursive: true });
		...
		it.skipIf(process.platform === "win32")("should load symlinked user and project extensions once", async () => {
			...
			writeFileSync(
				...
				`export default function(misul) {
	misul.registerCommand("shared", {
		...
		handler: async () => {},
			...
			mkdirSync(agentDir, { recursive: true });
			mkdirSync(join(cwd, ".misul"), { recursive: true });
			symlinkSync(sharedExtDir, join(agentDir, "extensions"), "dir");
			symlinkSync(sharedExtDir, join(cwd, ".misul", "extensions"), "dir");
			...
			expect(extensionsResult.extensions[0].path).toBe(join(cwd, ".misul", "extensions", "shared.ts"));
		});

		it("should load user extensions before trust and reuse them after trust resolves", async () => {
			const userExtDir = join(agentDir, "extensions");
			const projectExtDir = join(cwd, ".misul", "extensions");
...
export default function(misul) {
	...
	misul.registerCommand("user-trust", {
		...
		handler: async () => {},
			...
			writeFileSync(
				...
				`export default function(misul) {
	misul.registerCommand("project-trusted", {
		...
		handler: async () => {},
			...
			const extensionsResult = loader.getExtensions();
			expect(extensionsResult.extensions.map((extension) => extension.path)).toEqual([
				join(cwd, ".misul", "extensions", "project.ts"),
				join(userExtDir, "user.ts"),
			]);
	...
	misul.registerCommand("project-only", {
	...
	misul.registerCommand("user-only", {
...
Content`,
		...
		it("should discover SYSTEM.md from cwd/.misul", async () => {
			const piDir = join(cwd, ".misul");
