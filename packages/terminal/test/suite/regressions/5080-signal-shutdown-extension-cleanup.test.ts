...
// Regression for https://github.com/earendil-works/misul/issues/5080
//
...
function createSessionManager(options: { sessionFile?: string } = {}): SessionManager {
	return {
		...
		getSessionDir: () => "/tmp/misul-sessions",
		usesDefaultSessionDir: () => true,
	} as unknown as SessionManager;
}

function createTempFile(): string {
	const dir = mkdtempSync(join(tmpdir(), "misul-shutdown-resume-hint-"));
	tempDirs.push(dir);
