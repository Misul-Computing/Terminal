...
/**
 * Regression test for https://github.com/Misul-Computing/Terminal/issues/2791
 *
...
describe("issue #2791 fs.watch error event crashes process", () => {
	...
	beforeEach(() => {
		tempRoot = mkdtempSync(join(tmpdir(), "misul-2791-"));
		const agentDir = join(tempRoot, "agent");
		const themesDir = join(agentDir, "themes");
		mkdirSync(themesDir, { recursive: true });

		// Copy dark theme as "custom-test" theme
		const themesPath = join(__dirname, "../../../src/modes/interactive/theme/themes.json");
	...
	it("process should survive an error event on the theme FSWatcher", () => {
		...
		writeFileSync(
			scriptPath,
			`
import { setTheme, stopThemeWatcher } from "${pathToFileURL(themeModulePath).href}";

process.env.MISUL_TERMINAL_DIR = "${agentDir}";
...
`,
		...
		try {
			_stdout = execFileSync(process.execPath, [scriptPath], {
				timeout: 10000,
				encoding: "utf-8",
				env: { ...process.env, MISUL_TERMINAL_DIR: agentDir },
