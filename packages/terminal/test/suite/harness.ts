...
import type { AgentMessage, AgentTool } from "@misul/agent-core";
import { Agent } from "@misul/agent-core";
import type { FauxModelDefinition, FauxProviderRegistration, FauxResponseStep, Model } from "@misul/ai";
...
function createTempDir(): string {
	const tempDir = join(tmpdir(), `misul-suite-${Date.now()}-${Math.random().toString(36).slice(2)}`);
