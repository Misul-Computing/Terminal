...
const originalMisulPackageDir = process.env.MISUL_PACKAGE_DIR;
...
afterEach(() => {
	...
	if (originalMisulPackageDir === undefined) {
		delete process.env.MISUL_PACKAGE_DIR;
	} else {
		process.env.MISUL_PACKAGE_DIR = originalMisulPackageDir;
	}
...
function createNpmPrefixInstall(template = "misul-prefix-"): { prefix: string; packageDir: string } {
	const prefix = mkdtempSync(join(tmpdir(), template));
	const root = join(prefix, "lib", "node_modules");
	const scopeDir = join(root, "@earendil-works");
	const packageDir = join(scopeDir, "misul-coding-agent");
	mkdirSync(packageDir, { recursive: true });
	tempDir = prefix;
	process.env.MISUL_PACKAGE_DIR = packageDir;
	setExecPath(join(packageDir, "dist", "cli.js"));
	return { prefix, packageDir };
}

function createPnpmGlobalInstall(): { root: string; packageDir: string } {
	const temp = mkdtempSync(join(tmpdir(), "misul-pnpm-"));
	const binDir = join(temp, "bin");
	const root = join(temp, "pnpm", "global", "5", "node_modules");
	const packageDir = join(root, "@mariozechner", "misul-coding-agent");
	mkdirSync(packageDir, { recursive: true });
	...
	setExecPath(
		join(
			root,
			".pnpm",
			"@mariozechner+misul-coding-agent@0.0.0",
			"node_modules",
			"@mariozechner",
			"misul-coding-agent",
			"dist",
...
function createYarnGlobalInstall(): { globalDir: string; packageDir: string } {
	const temp = mkdtempSync(join(tmpdir(), "misul-yarn-"));
	const binDir = join(temp, "bin");
	const globalDir = join(temp, "yarn", "global");
	const packageDir = join(globalDir, "node_modules", "@mariozechner", "misul-coding-agent");
	mkdirSync(packageDir, { recursive: true });
	mkdirSync(binDir, { recursive: true });
	...
	process.env.PATH = `${binDir}${delimiter}${originalPath ?? ""}`;
	process.env.MISUL_PACKAGE_DIR = packageDir;
	setExecPath(join(globalDir, ".yarn", "@mariozechner", "misul-coding-agent", "dist", "cli.js"));
	return { globalDir, packageDir };
}

function createBunGlobalInstall(): { packageDir: string } {
	const temp = mkdtempSync(join(tmpdir(), "misul-bun-"));
...
describe("detectInstallMethod", () => {
	test("detects pnpm from Windows .pnpm install paths", () => {
		setExecPath(
			"C:\\Users\\Admin\\Documents\\pnpm-repository\\global\\5\\.pnpm\\@earendil-works+misul-coding-agent@0.67.68\\node_modules\\@earendil-works\\misul-coding-agent\\dist\\cli.js",
		);
	...
	test("self-updates renamed packages from the current install prefix", () => {
		...
		const command = getSelfUpdateCommand("@mariozechner/misul-coding-agent", undefined, "@new-scope/misul");

		expect(command).toEqual({
			command: "npm",
			args: ["--prefix", prefix, "install", "-g", "--ignore-scripts", "--min-release-age=0", "@new-scope/misul"],
			display: `npm --prefix ${prefix} uninstall -g @mariozechner/misul-coding-agent && npm --prefix ${prefix} install -g --ignore-scripts --min-release-age=0 @new-scope/misul`,
			steps: [
				{
					command: "npm",
					args: ["--prefix", prefix, "uninstall", "-g", "@mariozechner/misul-coding-agent"],
					display: `npm --prefix ${prefix} uninstall -g @mariozechner/misul-coding-agent`,
				},
				{
					command: "npm",
					args: ["--prefix", prefix, "install", "-g", "--ignore-scripts", "--min-release-age=0", "@new-scope/misul"],
					display: `npm --prefix ${prefix} install -g --ignore-scripts --min-release-age=0 @new-scope/misul`,
				},
			],
		});
	});
	...
	test("quotes npm self-update display paths", () => {
		const { prefix } = createNpmPrefixInstall("misul prefix ");
	...
	test("does not infer Windows npm custom prefixes from package paths", () => {
		const packageDir = "C:\\Users\\Admin\\npm prefix\\node_modules\\@earendil-works\\misul-coding-agent";
		process.env.MISUL_PACKAGE_DIR = packageDir;
	...
	test.skipIf(process.platform === "win32")("self-updates renamed pnpm global installs by removing the old package first", () => {
		...
		expect(command).toEqual({
			command: "pnpm",
			args: ["install", "-g", "--ignore-scripts", "--config.minimumReleaseAge=0", "@new-scope/misul"],
			display:
				"pnpm remove -g @mariozechner/misul-coding-agent && pnpm install -g --ignore-scripts --config.minimumReleaseAge=0 @new-scope/misul",
			steps: [
				{
					command: "pnpm",
					args: ["remove", "-g", "@mariozechner/misul-coding-agent"],
					display: "pnpm remove -g @mariozechner/misul-coding-agent",
				},
				{
					command: "pnpm",
					args: ["install", "-g", "--ignore-scripts", "--config.minimumReleaseAge=0", "@new-scope/misul"],
					display: "pnpm install -g --ignore-scripts --config.minimumReleaseAge=0 @new-scope/misul",
				},
			],
		});
	});
	...
	test.skipIf(process.platform === "win32")("self-updates pnpm v11 global installs resolved through the store", () => {
		const temp = mkdtempSync(join(tmpdir(), "misul-pnpm11-"));
		const binDir = join(temp, "bin");
		const root = join(temp, "Library", "pnpm", "global", "v11");
		const packageName = "@misul/terminal";
		const globalPackageDir = join(root, "11e9a", "node_modules", "@earendil-works", "misul-coding-agent");
		...
		tempDir = temp;
		process.env.PATH = `${binDir}${delimiter}${originalPath ?? ""}`;
		process.env.MISUL_PACKAGE_DIR = storePackageDir;
	...
	test.skipIf(process.platform === "win32")("self-updates renamed yarn global installs by removing the old package first", () => {
		...
		expect(command).toEqual({
			command: "yarn",
			args: ["global", "add", "--ignore-scripts", "@new-scope/misul"],
			display: "yarn global remove @mariozechner/misul-coding-agent && yarn global add --ignore-scripts @new-scope/misul",
			steps: [
				{
					command: "yarn",
					args: ["global", "remove", "@mariozechner/misul-coding-agent"],
					display: "yarn global remove @mariozechner/misul-coding-agent",
				},
				{
					command: "yarn",
					args: ["global", "add", "--ignore-scripts", "@new-scope/misul"],
					display: "yarn global add --ignore-scripts @new-scope/misul",
				},
			],
		});
	});
	...
	test.skipIf(process.platform === "win32")("self-updates renamed bun global installs by removing the old package first", () => {
		...
		expect(command).toEqual({
			command: "bun",
			args: ["install", "-g", "--ignore-scripts", "--minimum-release-age=0", "@new-scope/misul"],
			display:
				"bun uninstall -g @mariozechner/misul-coding-agent && bun install -g --ignore-scripts --minimum-release-age=0 @new-scope/misul",
			steps: [
				{
					command: "bun",
					args: ["uninstall", "-g", "@mariozechner/misul-coding-agent"],
					display: "bun uninstall -g @mariozechner/misul-coding-agent",
				},
				{
					command: "bun",
					args: ["install", "-g", "--ignore-scripts", "--minimum-release-age=0", "@new-scope/misul"],
					display: "bun install -g --ignore-scripts --minimum-release-age=0 @new-scope/misul",
				},
			],
		});
	});
