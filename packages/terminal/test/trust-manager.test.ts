...
describe("ProjectTrustStore", () => {
	...
	it.skipIf(process.platform === "win32")("detects trust-requiring project resources", () => {
		...
		try {
			mkdirSync(join(tempDir, ".misul", "agent"), { recursive: true });
			...
			writeFileSync(join(tempDir, ".misul", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(tempDir)).toBe(true);
			rmSync(join(tempDir, ".misul", "settings.json"), { force: true });

			mkdirSync(join(cwd, ".misul"), { recursive: true });
			writeFileSync(join(cwd, ".misul", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(cwd)).toBe(true);

			rmSync(join(cwd, ".misul"), { recursive: true, force: true });
