> misul can help you use the SDK. Ask it to build an integration for your use case.

# SDK

The SDK provides programmatic access to misul's agent capabilities. Use it to embed misul in other applications, build custom interfaces, or integrate with automated workflows.
...
**Behavior:**
- **Extension commands** (e.g., `/mycommand`): Execute immediately, even during streaming. They manage their own LLM interaction via `misul.sendMessage()`.
...
The `Agent` class (from `@misul/agent-core`) handles the core LLM interaction. Access it via `session.agent`.
...
const { session } = await createAgentSession({
  ...
  agentDir: "~/.misul/agent", // default (expands ~)
});
```

`cwd` is used by `DefaultResourceLoader` for:
- Project extensions (`.misul/extensions/`)
- Project skills:
  - `.misul/skills/`
  - `.agents/skills/` in `cwd` and ancestor directories (up to git repo root, or filesystem root when not in a repo)
- Project prompts (`.misul/prompts/`)
...
- Global skills:
  - `skills/` under `agentDir` (for example `~/.misul/agent/skills/`)
  - `~/.agents/skills/`
...
// Default: uses ~/.misul/agent/auth.json and ~/.misul/agent/models.json
const authStorage = AuthStorage.create();
const modelRegistry = ModelRegistry.create(authStorage);
...
```typescript
import { createAgentSession, DefaultResourceLoader } from "@misul/terminal";
...
The `edit` tool returns `details.diff` for Misul Terminal's TUI display and `details.patch` as a standard unified patch for SDK consumers.
...
import { createAgentSession, defineTool } from "@misul/terminal";
...
Use `defineTool()` for standalone definitions and arrays like `customTools: [myTool]`. Inline `misul.registerTool({ ... })` already infers parameter types correctly.

Custom tools passed via `customTools` are combined with extension-registered tools. Extensions loaded by the ResourceLoader can also register tools via `misul.registerTool()`.
...
Extensions are loaded by the `ResourceLoader`. `DefaultResourceLoader` discovers extensions from `~/.misul/agent/extensions/`, `.misul/extensions/`, and settings.json extension sources.
...
**Event Bus:** Extensions can communicate via `misul.events`. Pass a shared `eventBus` to `DefaultResourceLoader` if you need to emit or listen from outside:

```typescript
import { createEventBus, DefaultResourceLoader } from "@misul/terminal";
...
```typescript
import { createAgentSession, SettingsManager, SessionManager } from "@misul/terminal";
...
1. Global: `~/.misul/agent/settings.json`
2. Project: `<cwd>/.misul/settings.json`
