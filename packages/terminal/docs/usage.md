...
On Windows Terminal, Alt+Enter is fullscreen by default. Remap it as described in [Terminal setup](terminal-setup.md) if you want misul to receive the shortcut.

Configure delivery in [Settings](settings.md) with `steeringMode` and `followUpMode`.

## Sessions

Sessions are saved automatically to `~/.misul/agent/sessions/`, organized by working directory.

```bash
misul -c                  # Continue most recent session
misul -r                  # Browse and select a session
misul --no-session        # Ephemeral mode; do not save
misul --name "my task"    # Set session display name at startup
misul --session <path|id> # Use a specific session file or session ID
misul --fork <path|id>    # Fork a session into a new session file
```
...
- `.misul/SYSTEM.md` for a project
- `~/.misul/agent/SYSTEM.md` globally
...
misul update [source|self|misul]   # Update misul and packages; reconcile pinned git refs
misul update --extensions       # Update packages only; reconcile pinned git refs
...
These commands manage Misul packages, not the Misul Terminal CLI installation. To uninstall Misul Terminal itself, see [Quickstart](quickstart.md#uninstall). `misul config` and project package commands accept `--approve`/`--no-approve` to trust or ignore project-local settings for one command. `misul update` never prompts for project trust.

See [Misul Packages](packages.md) for package sources and security notes.
...
```bash
misul --no-extensions -e ./my-extension.ts
```
...
|----------|-------------|
| `MISUL_TERMINAL_DIR` | Override config directory; default is `~/.misul/agent` |
| `MISUL_TERMINAL_SESSION_DIR` | Override session storage directory; overridden by `--session-dir` |
| `MISUL_PACKAGE_DIR` | Override package directory, useful for Nix/Guix store paths |
...
| `MISUL_TELEMETRY` | Override install/update telemetry and provider attribution headers: `1`/`true`/`yes` or `0`/`false`/`no`. This does not disable update checks |
| `MISUL_CACHE_RETENTION` | Set to `long` for extended prompt cache where supported |
