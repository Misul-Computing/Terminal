> misul can create themes. Ask it to build one for your setup.
...
Misul Terminal loads themes from:

- Built-in: `dark`, `light`
- Global: `~/.misul/agent/themes/*.json`
- Project: `.misul/themes/*.json` (only after the project is trusted)
- Packages: `themes/` directories or `misul.themes` entries in `package.json`
...
On first run, misul detects your terminal background and defaults to `dark` or `light`.
...
```bash
mkdir -p ~/.misul/agent/themes
vim ~/.misul/agent/themes/my-theme.json
```
...
```json
{
  "$schema": "https://raw.githubusercontent.com/Misul-Computing/Terminal/main/packages/coding-agent/src/modes/interactive/theme/theme-schema.json",
  "name": "my-theme",
  "vars": {
    "primary": "#00aaff",
    "secondary": 242
...
**Hot reload:** When you edit the currently active custom theme file, misul reloads it automatically for immediate visual feedback.
...
Misul Terminal uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, misul falls back to the nearest approximation.
