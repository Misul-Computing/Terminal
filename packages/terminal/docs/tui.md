> misul can create TUI components. Ask it to build one for your use case.

# TUI Components

Extensions and custom tools can render custom TUI components for interactive user interfaces. This page covers the component system and available building blocks.

**Source:** [`@misul/tui`](https://github.com/Misul-Computing/Terminal/tree/main/packages/tui)
...
```typescript
import { CURSOR_MARKER, type Component, type Focusable } from "@misul/tui";
...
The cursor remains hidden by default. This keeps the fake cursor rendering, while still positioning the hardware cursor for terminals that track IME candidate windows with hidden cursors. Some terminals require a visible hardware cursor for IME positioning; enable it with `showHardwareCursor`, `setShowHardwareCursor(true)`, or `MISUL_HARDWARE_CURSOR=1`. The `Editor` and `Input` built-in components already implement this interface.
...
```typescript
import { Container, type Focusable, Input } from "@misul/tui";
...
**In custom tools** via `misul.ui.custom()`:

```typescript
async execute(toolCallId, params, onUpdate, ctx, signal) {
  const handle = misul.ui.custom(myComponent);
  // ...
  handle.close();
}
```
...
Import from `@misul/tui`:

```typescript
import { Text, Box, Container, Spacer, Markdown } from "@misul/tui";
```
...
```typescript
import { matchesKey, Key } from "@misul/tui";
...
```typescript
import { visibleWidth, truncateToWidth } from "@misul/tui";
...
```typescript
import { getMarkdownTheme } from "@misul/terminal";
import { Markdown } from "@misul/tui";
...
Set `MISUL_TUI_WRITE_LOG` to capture the raw ANSI stream written to stdout.

```bash
MISUL_TUI_WRITE_LOG=/tmp/tui-ansi.log npx tsx packages/tui/test/chat-simple.ts
```
...
For letting users pick from a list of options. Use `SelectList` from `@misul/tui` with `DynamicBorder` for framing.

```typescript
import type { ExtensionAPI } from "@misul/terminal";
import { DynamicBorder } from "@misul/terminal";
import { Container, type SelectItem, SelectList, Text } from "@misul/tui";
...
```typescript
import { BorderedLoader } from "@misul/terminal";

misul.registerCommand("fetch", {
  handler: async (_args, ctx) => {
    const result = await ctx.ui.custom<string | null>((tui, theme, _kb, done) => {
...
For toggling multiple settings. Use `SettingsList` from `@misul/tui` with `getSettingsListTheme()`.

```typescript
import { getSettingsListTheme } from "@misul/terminal";
import { Container, type SettingItem, SettingsList, Text } from "@misul/tui";

misul.registerCommand("settings", {
  handler: async (_args, ctx) => {
...
Customize the inline working indicator shown while misul is streaming a response.
...
// Restore misul's default spinner
ctx.ui.setWorkingIndicator();
```
...
import { matchesKey, truncateToWidth } from "@misul/tui";
