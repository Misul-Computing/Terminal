 ...
 *
 * Runs misul's built-in tools inside a local Gondolin micro-VM. The host working
 ...
 * Usage:
 *   cd /path/to/project
 *   misul -e /path/to/misul/packages/coding-agent/examples/extensions/gondolin
 *
 * Requirements:
...
export default function (api: ExtensionAPI) {
	...
	async function startVm(ctx?: ExtensionContext): Promise<VM> {
		...
		const created = await VM.create({
			sessionLabel: `misul ${path.basename(localCwd)}`,
			vfs: {
				mounts: {
	...
	api.on("user_bash", async (_event, ctx) => {
		const activeVm = await ensureVm(ctx);
