...
async function resolveGitHubRepo(api: ExtensionAPI, cwd: string): Promise<RepoResolution> {
	const result = await api.exec("git", ["remote", "-v"], { cwd, timeout: 5_000 });
	if (result.code !== 0) {
...
export default function (api: ExtensionAPI): void {
	api.on("session_start", async (_event, ctx) => {
		const resolvedRepo = await resolveGitHubRepo(api, ctx.cwd);
		if (!resolvedRepo.ok) {
