 ...
 *
 * Usage: misul --extension ./examples/extensions/overlay-qa-tests.ts
 *
 * Commands:
...
import type { ExtensionAPI, ExtensionCommandContext, Theme } from "@misul/terminal";
import type { Component, OverlayAnchor, OverlayHandle, OverlayOptions, TUI } from "@misul/tui";
import { Input, matchesKey, truncateToWidth, visibleWidth } from "@misul/tui";
...
export default function (api: ExtensionAPI) {
	// Animation demo - proves overlays can handle real-time updates (like doom would need)
	api.registerCommand("overlay-animation", {
	...
	api.registerCommand("overlay-anchors", {
	...
	api.registerCommand("overlay-margins", {
	...
	api.registerCommand("overlay-stack", {
	...
	api.registerCommand("overlay-overflow", {
	...
	api.registerCommand("overlay-edge", {
	...
	api.registerCommand("overlay-percent", {
	...
	// Test maxHeight
	api.registerCommand("overlay-maxheight", {
	...
	api.registerCommand("overlay-sidepanel", {
	...
	api.registerCommand("overlay-toggle", {
	...
	api.registerCommand("overlay-passive", {
	...
	api.registerCommand("overlay-focus", {
	...
	api.registerCommand("overlay-streaming", {
	...
	private startProcess(): void {
		...
		this.proc = spawn("bash", [
			...
			for i in $(seq 1 100); do
				...
				DIR="/Users/nicobailon/Documents/development/Terminal/packages/coding-agent/src/modes/interactive"
				...
				if [ $((i % 5)) -eq 0 ]; then
					echo -e "  \\033[32m✓ Successfully processed \${i} files in /Users/nicobailon/Documents/development/Terminal\\033[0m"
				fi
				if [ $((i % 7)) -eq 0 ]; then
...
// Animation demo - proves overlays can handle real-time updates like doom
class AnimationDemoComponent extends BaseOverlay {
	...
	render(width: number): string[] {
		...
		lines.push(border("│") + padLine(th.fg("dim", " (doom uses same approach)")) + border("│"));
		lines.push(border("│") + padLine(``) + border("│"));
