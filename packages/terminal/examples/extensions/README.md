...
Example extensions for misul terminal.

## Usage

```bash
# Load an extension with --extension flag
misul --extension examples/extensions/permission-gate.ts

# Or copy to extensions directory for auto-discovery
cp permission-gate.ts ~/.misul/agent/extensions/
```
...
| `send-user-message.ts` | Demonstrates `api.sendUserMessage()` for sending user messages from extensions |
...
| Extension | Description |
|-----------|-------------|
| `mac-system-theme.ts` | Syncs misul theme with macOS dark/light mode |
...
| `event-bus.ts` | Inter-extension communication via `api.events` |
...
| `custom-provider-gitlab-duo/` | GitLab Duo provider using @misul/ai's built-in Anthropic/OpenAI streaming via proxy |
