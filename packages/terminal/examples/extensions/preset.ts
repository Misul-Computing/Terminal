 ...
 * - ~/.misul/agent/presets.json (global)
 * - <cwd>/.misul/presets.json (project-local)
 *
 ...
 *
 * Usage:
 * - `misul --preset plan` - start with plan preset
...
function loadPresets(cwd: string): PresetsConfig {
	...
	const projectPath = join(cwd, ".misul", "presets.json");
...
export default function presetExtension(api: ExtensionAPI) {
	...
	async function applyPreset(name: string, preset: Preset, ctx: ExtensionContext): Promise<boolean> {
		...
		if (preset.thinkingLevel) {
			api.setThinkingLevel(preset.thinkingLevel);
		}
	...
	async function showPresetSelector(ctx: ExtensionContext): Promise<void> {
		...
		if (presetNames.length === 0) {
			ctx.ui.notify("No presets defined. Add presets to ~/.misul/agent/presets.json or .misul/presets.json", "warning");
			return;
		}
		...
		if (result === "(none)") {
			...
			if (originalState) {
				if (originalState.model) {
					await api.setModel(originalState.model);
				}
				api.setThinkingLevel(originalState.thinkingLevel);
				api.setActiveTools(originalState.tools);
			} else {
				api.setActiveTools(["read", "bash", "edit", "write"]);
			}
	...
	api.on("session_start", async (_event, ctx) => {
		...
		const presetFlag = api.getFlag("preset");
	...
	api.on("turn_start", async () => {
		if (activePresetName) {
			api.appendEntry("preset-state", { name: activePresetName });
		}
	});
}
