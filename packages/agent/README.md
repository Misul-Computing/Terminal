...
Stateful agent with tool execution and event streaming. Built on `@misul/ai`.

## Installation

```bash
npm install @misul/agent-core
```
...
```typescript
declare module "@misul/agent-core" {
...
```typescript
import { Agent, streamProxy } from "@misul/agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "@misul/agent-core";
