...
const { skills: sourcedSkills } = await loadSourcedSkills<Source, SourcedSkill>(
	env,
	[
		source("project", join(env.cwd, ".misul/skills")),
		source("user", join(homedir(), ".misul/agent/skills")),
		source("path", join(env.cwd, "../../../misul-skills")),
	],
	(skill, source) => ({ ...skill, source }),
);
const { promptTemplates: sourcedPromptTemplates } = await loadSourcedPromptTemplates<Source, SourcedPromptTemplate>(
	env,
	[source("project", join(env.cwd, ".misul/prompts")), source("user", join(homedir(), ".misul/agent/prompts"))],
