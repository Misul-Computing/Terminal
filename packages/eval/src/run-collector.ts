...
import type { AssistantMessage, Model, StopReason } from "@misul/ai";
...
export class AgentRunCollector {
	...
	beginChat(
		span: Span,
		init: { readonly stepNumber: number; readonly model: Model<string>; readonly provider?: string },
	): void {
	...
	endChat(
		...
		const reasoningOutputTokens = (usage as { reasoningTokens?: number })?.reasoningTokens ?? 0;
