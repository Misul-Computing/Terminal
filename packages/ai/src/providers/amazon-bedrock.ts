...
function addCustomHeadersMiddleware(client: BedrockRuntimeClient, headers: Record<string, string>): void {
	...
	client.middlewareStack.add(middleware, { step: "build", name: "misul-ai-custom-headers", priority: "low" });
}
 ...
 * Defaults to "short" and uses MISUL_CACHE_RETENTION for backward compatibility.
 */
		...
		const result: Record<string, any> = supportsAdaptiveThinking(model.id, model.name)
			...
			: (() => {
					const defaultBudgets: Record<ThinkingLevel, number> = {
						...
						max: 31999, // top thinking budget (matches OpenCode's "max")
					};
