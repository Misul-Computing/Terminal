...
const originalAgentDir = process.env.MISUL_TERMINAL_DIR;
...
afterEach(() => {
	...
	if (originalAgentDir === undefined) {
		...
		process.env.MISUL_TERMINAL_DIR = originalAgentDir;
	}
	resetOpenAICodexWebSocketDebugStats();
	vi.useRealTimers();
	vi.restoreAllMocks();
...
describe("openai-codex streaming", () => {
	it("streams SSE responses into AssistantMessageEventStream", async () => {
		const tempDir = mkdtempSync(join(tmpdir(), "misul-codex-stream-"));
		process.env.MISUL_TERMINAL_DIR = tempDir;
		...
		const fetchMock = vi.fn(async (input: string | URL, init?: RequestInit) => {
			...
			if (url === "https://chatgpt.com/backend-api/codex/responses") {
				...
				expect(headers?.get("originator")).toBe("misul");
