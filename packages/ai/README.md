...
```bash
npm install @misul/ai
```

TypeBox exports are re-exported from `@misul/ai`: `Type`, `Static`, and `TSchema`.

## Quick Start

```typescript
import { Type, getModel, stream, complete, Context, Tool, StringEnum } from '@misul/ai';
...
```typescript
import { Type, Tool, StringEnum } from '@misul/ai';
...
```typescript
import { stream, validateToolCall, Tool } from '@misul/ai';
...
```typescript
import { readFileSync } from 'fs';
import { getModel, complete } from '@misul/ai';
...
```typescript
import { getImageModel, generateImages } from '@misul/ai';
...
```typescript
import { getModel, streamSimple, completeSimple } from '@misul/ai';
...
```typescript
import { getModel, stream } from '@misul/ai';
...
```typescript
import { getProviders, getModels, getModel } from '@misul/ai';
...
```typescript
import { Model, stream } from '@misul/ai';
...
Use model-level `thinkingLevelMap` to describe model-specific thinking controls. Keys are misul thinking levels (`off`, `minimal`, `low`, `medium`, `high`, `xhigh`). Missing keys use provider defaults, string values are sent to the provider, and `null` marks a level unsupported.
...
```typescript
import { streamAnthropic, type AnthropicOptions } from '@misul/ai';
...
```typescript
import { getModel, complete, Context } from '@misul/ai';
...
```typescript
import { Context, getModel, complete } from '@misul/ai';
...
- OAuth login flows are not supported in browser environments. Use the `@misul/ai/oauth` entry point in Node.js.
...
Pass `env` in stream options to scope provider configuration to a request. Values in `env` are used before process environment variables for API key discovery and provider configuration such as Cloudflare account IDs, Azure OpenAI settings, Vertex project/location, Bedrock settings, `MISUL_CACHE_RETENTION`, and `HTTP_PROXY`/`HTTPS_PROXY`.
...
```typescript
import { getEnvApiKey } from '@misul/ai';
...
```bash
npx @misul/ai login              # interactive provider selection
npx @misul/ai login anthropic    # login to specific provider
npx @misul/ai list               # list available providers
```
...
The library provides login and token refresh functions via the `@misul/ai/oauth` entry point. Credential storage is the caller's responsibility.
...
```typescript
import { loginGitHubCopilot } from '@misul/ai/oauth';
...
```typescript
import { getModel, complete } from '@misul/ai';
import { getOAuthApiKey } from '@misul/ai/oauth';
...
- Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@misul/ai`
